...
exp_dir=Swamp_Moore_`date "+%Y-%m-%d_%H-%M-%S"` # directory name's formatting (date)
...
cp sweepM.sh $exp_dir # copy the drive script(.sh), into the directory
...
do
    ...
    do
	    ...
	    do
			...
			outfile="Swamp_Moore_Duck"$d"_Newt"$n"_Shrimp"$s"_Alga"$a"_Rock"$c"_Timesteps"$t".txt" # create & format the output file's name
			python3 swampLife.py M $d $n $s $a $c $t > $outfile # redirect the output into the output file(.txt)
		    done
		done
	    done
	done
